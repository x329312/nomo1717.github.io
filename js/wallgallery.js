		...
		create : function() {
			...
			this.$el.on( 'click', 'figure > div', function() {
				...
				if( self.caption === self.currentItem && idx === self.currentItem ) {
					...
					self.jump( idx, function() {
						self.showDescription( $textchild, idx );
					} );
				}
				else {
					self.showDescription( $textchild, idx );
